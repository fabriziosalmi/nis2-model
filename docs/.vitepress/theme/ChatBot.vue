...
<div v-else class="app">
  ...
  <div class="body">
    ...
    <aside v-if="showRight" class="side side-r">
      ...
      <div v-else class="empty-session">{{ t.emptySession }}</div>
    </aside>
  </div>
</div>
</template>
...
.metric{flex:1;text-align:center;padding:8px 4px;border-radius:6px;background:var(--vp-c-bg);border:1px solid var(--vp-c-divider)}
.m-val{font-size:16px;font-weight:700;font-variant-numeric:tabular-nums;letter-spacing:-.03em}
.m-label{font-size:8px;color:var(--vp-c-text-3);text-transform:uppercase;letter-spacing:.06em;margin-top:2px}

.trail{display:flex;flex-direction:column;gap:0}
.trail-btn{
  text-align:left;font-size:11px;color:var(--vp-c-text-3);
  padding:4px 8px;border:none;background:none;border-radius:5px;
  cursor:pointer;transition:all .12s;line-height:1.4;
  display:-webkit-box;-webkit-line-clamp:2;-webkit-box-orient:vertical;overflow:hidden;
}
.trail-btn:hover{background:rgba(59,130,246,.05);color:var(--vp-c-text-1)}
